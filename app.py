...
def datetime_processing(df):
	...
	df['date'] = (df.time - min_time) * (max_norm - min_norm) / (max_time - min_time) + min_norm
	return df
...
def logistic_page_builder(data):
	...
	accuracy_reg, f1_reg, roc_auc_reg, recall_reg, precision_reg, model_reg = logistic_train_metrics(data)
...
def xgb_page_builder(data):
	...
	start_time = datetime.datetime.now()
	accuracy_xgb, f1_xgb, roc_auc_xgb, recall_xgb, precision_xgb, model_xgb = XGB_train_metrics(data, params_set)
	st.subheader('Model Introduction')
	st.write('')
